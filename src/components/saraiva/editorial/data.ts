...
export function buildEditorialItems(articles: Article[], posts: BlogPost[]): EditorialCardItem[] {
  return [
    ...
    ...posts.map((post) => ({ id: post.id, href: `/blog/${post.slug}`, title: post.title, summary: post.excerpt, image: post.cover_image_url, label: "Blog", publishedAt: post.published_at })),
