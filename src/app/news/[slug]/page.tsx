...
export default async function ArticlePage({ params }: Props) { const article = await getArticleBySlug((await params).slug).catch(() => null); if (!article) notFound(); return <DetailPage label={sourceLabel(article.source_name)} title={article.title} date={article.published_at} brandedNewsArt externalUrl={article.url} externalLabel="Ler publicação original"><MarkdownContent content={article.summary} /></DetailPage>; }
