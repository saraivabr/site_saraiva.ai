...
export default async function NewsPage() {
  const [{ articles, posts }, offers] = await Promise.all([getEditorialData(), getPublicOffers()]);
  return <EditorialShell><main><EditorialHero eyebrow="01 · Radar" title="O que mudou enquanto você trabalhava." description="Notícias e análises com contexto suficiente para decidir — e personalidade suficiente para serem lembradas." /><NewsFeed items={buildEditorialItems(articles, posts)} /><OfferShowcase offers={offers.slice(0, 8)} /></main></EditorialShell>;
}
